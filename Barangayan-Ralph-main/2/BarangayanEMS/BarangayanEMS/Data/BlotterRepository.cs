    ...
    internal sealed class BlotterRepository
    {
        ...
        public List<BlotterRecord> GetAll()
        {
            ...
            using (var conn = new SQLiteConnection(DatabaseBootstrapper.ConnectionString))
            {
                ...
                {
                    cmd.CommandText = @"SELECT Id, CaseNo, ReportType, PriorityLevel, Barangay, Complainant, Respondent, IncidentDate, IncidentLocation, Description, Witnesses, Status, DateReported
FROM BlotterReports ORDER BY Id DESC";
